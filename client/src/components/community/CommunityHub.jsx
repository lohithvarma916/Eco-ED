...
export default function CommunityHub() {
    ...
    }).data, forumPosts = _a === void 0 ? [] : _a;
    var _b = useQuery({
        queryKey: ["/api/leaderboard"],
    }).data, leaderboard = _b === void 0 ? [] : _b;
    return (<div>
      <div className="mb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <Card className="p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <Button className="bg-student-primary hover:bg-student-dark text-white" data-testid="button-new-post" onClick={function () { window.location.href = '/discussions'; }}>
                <i className="fas fa-plus mr-2"></i>New Post
              </Button>
            </div>
            
            {forumPosts.length === 0 ? (<div className="text-center py-12">
                ...
                <Button className="bg-student-primary hover:bg-student-dark text-white" data-testid="button-start-discussion" onClick={function () { window.location.href = '/discussions'; }}>
                  Start Discussion
                </Button>
                    ...
                    <div className="flex items-start space-x-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2">
                          <h4 className="font-semibold" data-testid={"text-post-author-".concat(post.id)}>
                        ...
                        <h3 className="font-semibold mb-2" data-testid={"text-post-title-".concat(post.id)}>
                          {post.title}
                        </h3>
                        <p className="text-gray-600 mb-3">{post.content}</p>
                        <div className="flex items-center space-x-4 text-sm text-gray-500">
                          <button className="hover:text-student-primary" data-testid={"button-like-post-".concat(post.id)}>
                            <i className="fas fa-thumbs-up mr-1"></i>{post.likes} likes
                          </button>
                          <button className="hover:text-student-primary" data-testid={"button-reply-post-".concat(post.id)}>
        ...
        <div className="space-y-6">
          <Card className="p-6">
            ...
            {leaderboard.length === 0 ? (<div className="text-center py-6">
                <i className="fas fa-trophy text-gray-300 text-4xl mb-2"></i>
                <p className="text-gray-500 text-sm">No rankings yet</p>
              </div>) : (<div className="space-y-4">
                {leaderboard.slice(0, 3).map(function (user, index) { return (<div key={user.id} className="flex items-center space-x-3">
                    <div className={"w-8 h-8 rounded-full flex items-center justify-center ".concat(index === 0 ? 'bg-yellow-500' : index === 1 ? 'bg-gray-400' : 'bg-orange-500')}>
                      <span className="text-white text-sm font-bold">{index + 1}</span>
                    </div>
                    <div className="flex-1">
                      <p className="font-semibold text-sm" data-testid={"text-leader-name-".concat(index)}>
                        {user.firstName ? "".concat(user.firstName, " ").concat(user.lastName || '').trim() : 'Student'}
                      </p>
